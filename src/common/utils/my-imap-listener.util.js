...
import MyParser from './imap-parser.util.js';

const mailParser = MailParser.MailParser;
const eventEmitter = EventEmitter.EventEmitter;
const myParser = new MyParser();
export default class SimpleImapMail extends eventEmitter {
  constructor(options) {
    super();
    this.uids = [];
      ...
      this.imap.search([['UID', uid]], async (err, results) => {
        ...
        if (results.length > 0) {
          ...
          messageFetchQuery.on('message', (message, sequenceNumber) => {
            ...
            parser.on('headers', headers => myParser.parseHeaders(headers));

            parser.on('data', data => myParser.parseMessageData(data));

            parser.on('end', () => {
              this.emit('message', myParser.getParseResult());
              myParser.parseResult.attachments = [];
